...
function getMailAdres($typegebruikerid, $id)
{
    global $dbh;
...
function verstuurFeedbackMail($VerkopersID, $BeoordelersID){
    ...
    $to = getMailAdres('beoordelerID',$BeoordelersID);
    ...
    $message .= 'Om contact op te nemen met de verkoper kunt u hem/haar via ' . getMailAdres('verkopersID', $VerkopersID) . ' bereiken. ';
    $message .= '</body></html>';
